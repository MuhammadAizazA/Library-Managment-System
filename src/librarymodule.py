...
def save_everything(fiction_objects,non_fiction_objects,borrwoers_objects):
    bookmodule.save_all_to_file(fiction_objects, 'Data/fictionbooks.csv')
    bookmodule.save_all_to_file(non_fiction_objects, 'Data/nonfictionbooks.csv')
    ...
    while True:
        ...
        if interface_choice == 'LIB':
            ...
            while (True):
                print('----------------------------------------------------')
                print(
                    f'Hi {read_data.get_name()} You can perform the following operations')
                ...
                if choice == '1':
                    ...
                    while True:
                        ...
                        if len(isbn) == 17:
                            ...
                            print(
                                'Error: Lenght should be 13 with 4 (-) ie: 978-123-123-123-2')
                    ...
                    if genre >= 1 and genre < 5:
                        obj = bookmodule.FictionBook(
                            title, author, isbn, num_of_copies, genre_dict[genre])
                        obj.save_to_file()
                    elif genre >= 5 and genre <= 7:
                        obj = bookmodule.NonFictionBook(
                            title, author, isbn, num_of_copies, genre_dict[genre])
                        obj.save_to_file()
                elif choice == '2':
        ...
        # This next condition will start the user as browwer
            ...
            nonfiction_dict = bookmodule.load_books_into_list_objects(
                path_nonfic)
            ...
            while True:
                ...
                if browwer_choice == '1':
                    requested_isbn = input(
                        'Enter the ISBN you want to Borrow: ')

                    for book_set in [fiction_objects, non_fiction_objects]:
                        for book in book_set:
                            if requested_isbn == book.get_isbn():
                                logged_in_user.borrow_book(
                                    requested_isbn, book_set)
                                break
                        else:
                            continue
                        break
                    ...
                    returned_isbn = input(
                        'Enter the ISBN you want to Return: ')

                    for book_set in [fiction_objects, non_fiction_objects]:
                        for book in book_set:
                            if returned_isbn == book.get_isbn():
                                logged_in_user.return_book(
                                    returned_isbn, book_set)
                                break
                        else:
                            continue
                        break
                    ...
                    save_everything(fiction_objects,non_fiction_objects,borrwoers_objects)
                elif browwer_choice == '5':
                    print(
                        f'''Borrowed Books ISBN's:{logged_in_user.get_borrowed_isbns()}''')
